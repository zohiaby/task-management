...
const Dashboard = () => {
  ...
  const dashBoardData = useGetDasboardStatsQuery();
  ...
  const stats = [
    {
      _id: "1",
      label: "TOTAL TASK",
      total: dashBoardData?.currentData?.totalTasks || 0,
      icon: <FaNewspaper />,
      bg: "bg-[#1d4ed8]",
    },
    {
    ...
    <div className="h-full py-4">
      <>
        ...
        <div className="w-full bg-white my-16 p-4 rounded shadow-sm">
          ...
          <Chart data={dashBoardData?.currentData?.graphData} />
        </div>
        <div className="w-full flex flex-col md:flex-row gap-4 2xl:gap-10 py-8">
          {/* RECENT AUTHORS */}
          {data && <TaskTable tasks={dashBoardData?.currentData?.last10Task} />}
          {/* RECENT USERS */}
          {data && user?.isAdmin && (
            <UserTable users={dashBoardData?.currentData?.users} />
          )}
        </div>
      </>
    </div>
